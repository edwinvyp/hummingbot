...
def maker_trading_pair_prompt():
    exchange = day_trade_making_config_map.get("exchange").value
    example = EXAMPLE_PAIRS.get(exchange)
    return "Enter trading pair you would like to trade on %s%s >>> " \
...
async def order_amount_prompt() -> str:
    exchange = day_trade_making_config_map["exchange"].value
    trading_pair = day_trade_making_config_map["market"].value
    base_asset, quote_asset = trading_pair.split("-")
    min_amount = await minimum_order_amount(exchange, trading_pair)
    return f"Enter amount of {base_asset} your order? (minimum {min_amount}) >>> "
...
def on_validate_price_source(value: str):
    if value != "external_market":
        day_trade_making_config_map["price_source_exchange"].value = None
        day_trade_making_config_map["price_source_market"].value = None
        day_trade_making_config_map["take_if_crossed"].value = None
    if value != "custom_api":
        day_trade_making_config_map["price_source_custom_api"].value = None
    else:
        day_trade_making_config_map["price_type"].value = None


def price_source_market_prompt() -> str:
    external_market = day_trade_making_config_map.get("price_source_exchange").value
    return f'Enter trading pair on {external_market} >>> '


def validate_price_source_exchange(value: str) -> Optional[str]:
    if value == day_trade_making_config_map.get("exchange").value:
...
def validate_price_source_market(value: str) -> Optional[str]:
    market = day_trade_making_config_map.get("price_source_exchange").value
...
def on_validated_price_type(value: str):
    if value == 'inventory_cost':
        day_trade_making_config_map["inventory_price"].value = None
...
day_trade_making_config_map = {
    "strategy":
        ConfigVar(key="strategy",
                  prompt=None,
                  default="day_trade_making"),
    "exchange":
        ConfigVar(key="exchange",
                  prompt="Enter your maker spot connector ie: kraken >>> ",
    ...
    "bid_spread":
        ConfigVar(key="bid_spread",
                  prompt="enter price bid spread from mid price "
    ...
    "ask_spread":
        ConfigVar(key="ask_spread",
                  prompt="enter price ask spread from mid price "
    ...
    "order_level_amount":
        ConfigVar(key="order_level_amount",
                  ...
                  required_if=lambda: day_trade_making_config_map.get("order_levels").value > 1,
                  type_str="decimal",
    ...
    "inventory_target_base_pct":
        ConfigVar(key="inventory_target_base_pct",
                  ...
                  required_if=lambda: day_trade_making_config_map.get("inventory_skew_enabled").value,
                  type_str="decimal",
    ...
    "inventory_price":
        ConfigVar(key="inventory_price",
                  ...
                  required_if=lambda: day_trade_making_config_map.get("price_type").value == "inventory_cost",
                  default=Decimal("1"),
                  ),
    "filled_order_delay":
    ...
    "hanging_orders_cancel_pct":
        ConfigVar(key="hanging_orders_cancel_pct",
                  prompt="At what spread percentage (from mid price) will hanging orders be canceled? "
                         "(Enter 1 to indicate 1%) >>> ",
                  required_if=lambda: day_trade_making_config_map.get("hanging_orders_enabled").value,
                  type_str="decimal",
                  default=Decimal("10"),
                  validator=lambda v: validate_decimal(v, 0, 100, inclusive=False)),
    ...
    "ask_order_optimization_depth":
        ConfigVar(key="ask_order_optimization_depth",
                  ...
                  required_if=lambda: day_trade_making_config_map.get("order_optimization_enabled").value,
                  type_str="decimal",
    ...
    "price_type":
        ConfigVar(key="price_type",
                  ...
                  type_str="str",
                  required_if=lambda: day_trade_making_config_map.get("price_source").value != "custom_api",
                  default="mid_price",
    ...
    "price_source_exchange":
        ConfigVar(key="price_source_exchange",
                  ...
                  required_if=lambda: day_trade_making_config_map.get("price_source").value == "external_market",
                  type_str="str",
    ...
    "take_if_crossed":
        ConfigVar(key="take_if_crossed",
                  ...
                  required_if=lambda: day_trade_making_config_map.get(
